...
def step_impl(context):
    context.my_object.image_message(original_content_url=context.active_outline["original_content_url"],
...
@when(u'(Facebook) an image message is added')
def step_impl(context):
    ...
    if url != "-":
        kwargs["url"] = url
    print(kwargs)
    context.my_object.image_message(**kwargs)


@then(u'(Facebook) image message must have attachment_id = "{att_id}" or url = "{url}"')
def step_impl(context, att_id, url):
    ...
    for chatbot in chatbots:
        ...
        element = elements[0]
        assert element.get("media_type", None) == "image"
        if att_id != "-":
