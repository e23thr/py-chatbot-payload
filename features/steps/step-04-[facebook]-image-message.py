...
def step_impl(context):
    context.my_object.image_message(
        image_url=context.active_outline["url"],
        is_reusable=context.active_outline["is_reusable"] == "true",
        chat_bot="facebook"
    )


@then(u'(Facebook) image message must have url = "{url}" and is_reusable = "{is_reusable}"')
def step_impl(context, url, is_reusable):
    ...
    for chatbot in chatbots:
        ...
        assert obj_attachment.get("type", None) == "image"
